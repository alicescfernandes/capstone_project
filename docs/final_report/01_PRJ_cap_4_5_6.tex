...
A acessibilidade foi uma preocupação central no desenvolvimento da aplicação, e para isso foram seguidas as diretrizes WCAG 2.1 (Web Content Accessibility Guidelines), que definem critérios técnicos para tornar os conteúdos web mais acessíveis a todos os utilizadores, incluindo pessoas com deficiências visuais, motoras, auditivas, cognitivas ou neurológicas. Para garantir a conformidade com estas diretrizes, recorremos a uma abordagem prática combinada de testes automáticos e validação manual. Entre as ferramentas utilizadas destacam-se duas principais: o Lighthouse e o Axe.

O Lighthouse, desenvolvido pela Google, é uma ferramenta open source que corre diretamente no Chrome DevTools ou como CLI. Permite auditar uma página web em várias categorias, nomeadamente Performance, SEO, Best Practices e, claro, Accessibility. A secção de acessibilidade do Lighthouse verifica, por exemplo, se os elementos têm contrastes suficientes, se os formulários estão corretamente etiquetados com aria-label, se há títulos estruturados (h1, h2, etc.) numa hierarquia lógica e se os elementos interativos (como botões e links) são acessíveis via teclado. O Lighthouse fornece uma pontuação de 0 a 100 e destaca problemas comuns com sugestões práticas de como resolver.

Esta ferramenta Lighthouse, apesar de não ser tão detalhada como a Axe, sinalizou alguns problemas, como a falta de compressão Gzip e alguns erros de acessibilidade e SEO. Estes problemas foram resolvidos, e a ferramenta Lighthouse passou a dar uma pontuação melhor nos vários critérios que avalia.
...
Após resolver os problemas indicados pela ferramenta Axe, conseguimos então melhorar a acessibilidade da aplicação
...
Desta forma, procurámos alinhar a aplicação com boas práticas de desenvolvimento inclusivo, respeitando não só normas técnicas mas também a responsabilidade social de criar interfaces acessíveis a todos.
...
\begin{itemize}
    ...
    \item Estruturação semântica do HTML
    \item Suporte a leitores de ecrã
\end{itemize}

\section{Testes Manuais}

Para complementar os testes automatizados, foi desenvolvido um conjunto abrangente de testes manuais utilizando a linguagem Gherkin para descrever os cenários de teste. Cinco testadores independentes foram recrutados para executar estes testes, cada um focando em diferentes aspectos da aplicação.

Os cenários de teste foram organizados em três categorias principais:

\begin{itemize}
    \item Fluxos de utilizador básicos (criação de conta, login, navegação)
    \item Operações com dados (upload de ficheiros, visualização de gráficos)
    \item Casos de erro e recuperação
\end{itemize}

Cada cenário foi documentado usando a sintaxe Given-When-Then do Gherkin, permitindo uma clara compreensão dos pré-requisitos, ações e resultados esperados. Por exemplo:

\begin{verbatim}
Given um utilizador autenticado
When o utilizador carrega um ficheiro Excel
Then o sistema deve processar o ficheiro
And mostrar uma mensagem de sucesso
\end{verbatim}

Os resultados dos testes manuais revelaram algumas áreas para melhoria, particularmente na experiência de utilizador em dispositivos móveis e na clareza das mensagens de erro.

\section{Testes Automáticos}

Além dos testes de acessibilidade com Lighthouse, foram implementados outros testes automatizados para garantir a qualidade da aplicação:

\begin{itemize}
    \item Testes de integração para validar a comunicação entre frontend e backend
    \item Testes de performance para garantir tempos de resposta adequados
    \item Testes de segurança para identificar potenciais vulnerabilidades
    \item Testes de compatibilidade com diferentes navegadores
\end{itemize}

Os resultados dos testes de performance mostraram tempos de carregamento consistentes, com uma média de 2.5 segundos para o carregamento inicial da página e 1.2 segundos para o carregamento de gráficos individuais.
...
A aplicação demonstrou compatibilidade total com todos os navegadores modernos, com exceção do Internet Explorer 11, que não é suportado devido a limitações técnicas e à baixa utilização atual deste navegador.
